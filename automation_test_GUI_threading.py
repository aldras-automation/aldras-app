...
def on_press_recording(key):
    ...
    if 'key.ctrl_r' in output and ctrls >= 3:  # toggle recording
        ...
        print()
        print('RECORDING = {}'.format(recording_control))
        if not recording_control:
            print('Complete!')
            compile_recording()
            raise SystemExit()
...
def record(workflow_name_in):
    ...
    if recording:
        ...
        print('Primed for recording. To start recording, press the right CTRL three times.')
    else:
...
def gui_func(num):
    ...
    record_button = tk.Button(m, text=record_button_text, width=25, command=lambda: record(workflow_name_entry.get()))
...
if __name__ == '__main__':
    ...
    t2 = threading.Thread(target=listener_func, args=(10,))
    t1.start()
    t2.start()
    t1.join()
    t2.join()
